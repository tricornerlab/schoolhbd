...
@section('content')
    ...
    @php $title = __('sections.events'); $link = url('events') ;   @endphp
                            ...
                            <div class="">
                                <div class="flex flex-col items-center">
                                    ...
                                    <div class="font3 text-xl text-[#7fa7cb] pl-4">
                                        {{ $range[0][ 'title_'.app()->getLocale() ] }}
                                    </div>
                                    @if($range[0]['img'])
                                        <img src="{{ env('APP_URL').$range[0]['img'] }}" class="w-[400px] h-auto  m-4">
                                    @endif
                                </div>
                                <p class="font3 pl-12 text-gray-800">
                                    {{ $range[0]['content_'.app()->getLocale()] }}
                                </p>
...
{{--                                        DATE--}}
                                        ...
                                        <p class=" ">
                                            {{ substr($range[0]['created_at'], 0, 10) }}
                                        </p>

{{--                                        LIKES--}}
                                        <form action="{{ url('elike') }}" method="GET">
                                            <input class="hidden" name="id" value="{{ $range[0]['id'] }}">
                                            <button type="submit">
